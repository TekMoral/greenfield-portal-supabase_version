...
const QuestionRow = ({ q, idx, onChange, onRemove, onlyShortAnswer = false }) => {
  return (
      ...
      {q.type === 'true_false' && (
        <div className="flex items-center gap-3 text-sm">
          <label className="flex items-center gap-1">
            <input type="radio" name={`tf-${idx}`} checked={q.correct_answer === true} onChange={() => onChange({ correct_answer: true })} /> True
          </label>
          <label className="flex items-center gap-1">
            <input type="radio" name={`tf-${idx}`} checked={q.correct_answer === false} onChange={() => onChange({ correct_answer: false })} /> False
          </label>
        </div>
      )}
...
const QuestionsEditorModal = ({ open, assignment, state, onAdd, onUpdate, onRemove, onSave, onClose }) => {
  if (!open || !assignment) return null;
  const subject = assignment?.subjectName || assignment?.subjects?.name;
  const className = assignment?.classes?.name || assignment?.class_name;
