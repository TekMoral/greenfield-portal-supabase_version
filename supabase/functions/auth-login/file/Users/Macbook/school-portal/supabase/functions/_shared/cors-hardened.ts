...
function getCorsConfig() {
  ...
  const envAllowedOrigins = envOriginsRaw.split(',').map((o)=>o.trim()).filter(Boolean);
  const base = (()=>{
    switch(environment){
      case 'production':
      case 'prod':
...
export function getHardenedCorsHeaders(req) {
...
export function handleHardenedCors(req) {
...
export function getResponseCorsHeaders(req) {
...
export function validateRequest(req) {
  ...
  if (method !== 'GET' && method !== 'OPTIONS') {
    if (!origin) {
      return {
        valid: false,
        reason: 'Origin header required for non-GET requests'
      };
    }
    if (!isOriginAllowed(origin, config.allowedOrigins)) {
      return {
